...
const Dashboard = () => {
  ...
  const reverseGeocode = async (lat, lon) => {
    try {
      ...
      const district = data.address.state_district; // District for India
      const state = data.address.state; // State for India
  ...
  const fetchWeatherData = async (lat, lon) => {
    ...
    try {
      ...
      setWeather({
        ...
        rainfall: data.clouds.all, // Proxy for rainfall chance
  ...
  const fetchLocationAndData = async () => {
    ...
    try {
      ...
      await fetchWeatherData(18.5204, 73.8567); // Pune coordinates
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-green-200 dark:border-green-700">
                            <th className="text-left py-2 px-4 text-green-700 dark:text-green-400 font-medium">{t.crop}</th>
                            <th className="text-left py-2 px-4 text-green-700 dark:text-green-400 font-medium">{t.commodity}</th>
